...
const ProfileSection: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 card-shadow overflow-hidden flex flex-col">
      {/* Cittadinanza */}
      <div className="px-5 pt-6 pb-2">
        <label htmlFor="citizenship-select" className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Cittadinanza</label>
        <div className="relative group">
          <select 
            id="citizenship-select"
            value={profile.citizenship}
            onChange={(e) => updateProfile({ citizenship: e.target.value as Citizenship })}
            className="w-full bg-slate-50 border-slate-200 rounded-xl py-3 px-4 text-[15px] font-semibold text-slate-800 focus:ring-primary focus:border-primary appearance-none cursor-pointer"
          >
      ...
      <div className="px-5 py-3">
        <label htmlFor="license-category-select" className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Categoria Patente</label>
        <div className="relative group">
          <select 
        ...
        {profile.licenseCategory === LICENSE_CATEGORIES.AM && (
          <div className="mt-4">
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
              <span className="material-symbols-rounded text-primary font-bold">info</span>
              <div className="text-[13px] leading-relaxed">
                <p className="font-black text-primary mb-1">Nota per cat. AM</p>
                <p className="text-slate-600 font-medium">A pagina 4 del modello TT2112 specificare tipo veicolo e cambio. <u className="decoration-primary">È richiesta doppia firma (candidato + genitore) su tale nota.</u></p>
              </div>
            </div>
          </div>
        )}
